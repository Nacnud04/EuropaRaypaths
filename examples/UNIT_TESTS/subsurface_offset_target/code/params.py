...
maxZ = 300e3 # maximum altitude of source path [m]
...
sz = pg.vert_source_path(params, minZ, maxZ, "source_path", xoff=2000)

# --- RX OPENING WINDOW FILE ---
for depth in tdepths:
    pg.track_Z_rxwin(sz, depth * np.sqrt(params['eps_2']) + 500, f"rx_window_positions_{abs(int(depth)):04d}")
